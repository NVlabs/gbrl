...
void get_matrix_representation_cuda(dataSet *dataset, ensembleMetaData *metadata, ensembleData *edata, SGDOptimizerGPU** opts, const int n_opts, int start_tree_idx, int stop_tree_idx, matrixRepresentation *matrix){
    ...
    bool *device_A;
    // assuming row-major order
    size_t A_size = dataset->n_samples * (n_leaves+1) * sizeof(bool);
    size_t V_size = (n_leaves+1) * metadata->output_dim*sizeof(float);
    size_t obs_matrix_size = dataset->n_samples * metadata->n_num_features * sizeof(float);
    size_t cat_obs_matrix_size = dataset->n_samples * metadata->n_cat_features * sizeof(char) * MAX_CHAR_SIZE;
    ...
    if (metadata->grow_policy == GREEDY){
        if (metadata->n_cat_features == 0)
            get_representation_kernel_numerical_only<<<n_leaves, threads_per_block>>>(device_batch_obs, dataset->n_samples, metadata->n_num_features, edata->feature_indices, edata->depths, edata->feature_values, edata->inequality_directions, edata->values, opts, n_opts, metadata->output_dim, metadata->max_depth, start_leaf_idx, n_leaves, device_A, device_V);
        else
        ...
        if (metadata->n_cat_features == 0)
            ...
            get_representation_oblivious_kernel_tree_wise<<<n_trees, threads_per_block>>>(device_batch_obs, device_batch_cat_obs, dataset->n_samples, metadata->n_num_features, metadata->n_cat_features, edata->feature_indices, edata->depths, edata->feature_values, edata->inequality_directions, edata->values, edata->tree_indices, edata->categorical_values, edata->is_numerics, opts, n_opts, metadata->output_dim, metadata->max_depth, start_tree_idx, n_leaves, device_A, device_V);
    }
    cudaDeviceSynchronize();
    matrix->A = new bool[A_size];
    cudaMemcpy(matrix->A, device_A, A_size, cudaMemcpyDeviceToHost);
    for (int i = 0; i < n_samples; i++)
        matrix->A[i*(n_leaves + 1)] = true;
...
__global__ void get_representation_oblivious_kernel_tree_wise(const float* __restrict__ obs, const char* __restrict__ categorical_obs, const int n_samples, const int n_num_features, const int n_cat_features, 
    ...
    bool decision;
    int tree_idx = blockIdx.x + tree_offset;
    int leaf_idx, initial_leaf_idx = __ldg(tree_indices + tree_idx), relative_leaf_idx =  __ldg(tree_indices + blockIdx.x);
    for (int sample_idx = threadIdx.x; sample_idx < n_samples; sample_idx += blockDim.x){
        leaf_idx = 0;
        for (int depth_idx = 0; depth_idx < __ldg(depths + tree_idx); depth_idx++){ 
            if (is_numerics[tree_idx * max_depth + depth_idx])
        ...
        A[sample_idx*(n_leaves + 1) + leaf_idx + 1 + relative_leaf_idx] = true;
        if (n_opts == 1){
            for (int i = opts[0]->start_idx; i < opts[0]->end_idx; ++i){
                V[(leaf_idx + relative_leaf_idx + 1)*output_dim + i] = -opts[0]->init_lr * __ldg(leaf_values + (initial_leaf_idx + leaf_idx)*output_dim + i);
            }
        } else {
            for (int i = opts[0]->start_idx; i < opts[0]->end_idx; ++i){
                 V[(leaf_idx + relative_leaf_idx + 1)*output_dim + i] = -opts[0]->init_lr * __ldg(leaf_values + (initial_leaf_idx + leaf_idx)*output_dim + i);
            ...
            for (int i = opts[1]->start_idx; i < opts[1]->end_idx; ++i){
                 V[(leaf_idx + relative_leaf_idx + 1)*output_dim + i] = -opts[1]->init_lr * __ldg(leaf_values + (initial_leaf_idx + leaf_idx)*output_dim + i);
            }
        }
    }
}
...
__global__ void get_representation_oblivious_kernel_numerical_only(const float* __restrict__ obs, const int n_samples, const int n_num_features, 
    ...
    int leaf_idx, initial_leaf_idx = __ldg(tree_indices + blockIdx.x + tree_offset), relative_leaf_idx = __ldg(tree_indices + blockIdx.x);
    for (int sample_idx = threadIdx.x; sample_idx < n_samples; sample_idx += blockDim.x){
        leaf_idx = 0;
        for (int depth_idx = 0; depth_idx < __ldg(depths + blockIdx.x + tree_offset); depth_idx++){ 
            bool decision = (__ldg(&obs[sample_idx*n_num_features + __ldg(feature_indices + (blockIdx.x + tree_offset) * max_depth + depth_idx)])) > (__ldg(feature_values + (blockIdx.x + tree_offset) * max_depth + depth_idx));
