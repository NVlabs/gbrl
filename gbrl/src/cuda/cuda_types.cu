...
ensembleData* ensemble_data_alloc_cuda(ensembleMetaData *metadata){
    ...
    trace += value_sizes;
    edata->lambda_objs = (float *)(data + trace);
    ...
    edata->densities = (float *)(data + trace);
...
ensembleData* ensemble_data_copy_gpu_gpu(ensembleMetaData *metadata, ensembleData *other_edata, ensembleData *edata){
    ...
    cudaMemcpy(edata->lambda_objs, other_edata->lambda_objs, lambda_objs_size, cudaMemcpyDeviceToDevice);
    ...
    cudaMemcpy(edata->densities, other_edata->densities, densities_size, cudaMemcpyDeviceToDevice);
...
ensembleData* ensemble_data_copy_gpu_cpu(ensembleMetaData *metadata, ensembleData *other_edata, ensembleData *edata){
    ...
    cudaMemcpy(edata->lambda_objs, other_edata->lambda_objs, lambda_objs_size, cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(edata->densities, other_edata->densities, densities_size, cudaMemcpyDeviceToHost);
...
ensembleData* ensemble_data_copy_cpu_gpu(ensembleMetaData *metadata, ensembleData *other_edata, ensembleData *edata){
    ...
    cudaMemcpy(edata->lambda_objs, other_edata->lambda_objs, lambda_objs_size, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(edata->densities, other_edata->densities, densities_size, cudaMemcpyHostToDevice);
...
void allocate_ensemble_memory_cuda(ensembleMetaData *metadata, ensembleData *edata){
    ...
    if  ((leaf_idx >= metadata->max_leaves) || (tree_idx >= metadata->max_trees)){
        ...
        cudaMemcpy(new_data->densities, edata->densities, leaf_idx * metadata->n_objs * sizeof(float), cudaMemcpyDeviceToDevice);
        cudaMemcpy(new_data->lambda_objs, edata->lambda_objs, metadata->n_objs * sizeof(float), cudaMemcpyDeviceToDevice);
        if (metadata->grow_policy == GREEDY){
