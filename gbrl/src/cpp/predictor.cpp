...
void Predictor::get_matrix_representation_cpu(dataSet *dataset, const ensembleData *edata, const ensembleMetaData *metadata, int start_tree_idx, int stop_tree_idx, const bool parallel_predict, matrixRepresentation *matrix, std::vector<Optimizer*> opts){
    const int par_th = metadata->par_th, n_samples = dataset->n_samples;

    int start_leaf_idx = edata->tree_indices[start_tree_idx];
    int stop_leaf_idx = (stop_tree_idx < metadata->n_trees) ? edata->tree_indices[stop_tree_idx]: metadata->n_leaves;
    int n_leaves = stop_leaf_idx - start_leaf_idx;
    // first column is all ones for including bias
    bool *A = new bool[n_samples*(n_leaves + 1)];
    memset(A, 0, n_samples*(n_leaves + 1));
    for (int i = 0; i < n_samples; i++)
        A[i*(n_leaves + 1)] = true;
    matrix->A = A;
    matrix->V = new float[metadata->output_dim*(n_leaves + 1)];
    ...
    void (*getRepresentationFunc)(const float*, const char*, const int, const ensembleData*, const ensembleMetaData*, const int, const int,  const int, std::vector<Optimizer*>, matrixRepresentation*) = nullptr;
    getRepresentationFunc = (metadata->grow_policy == OBLIVIOUS) ? &Predictor::get_representation_matrix_over_trees : &Predictor::get_representation_matrix_over_leaves;
    int n_tree_threads = calculate_num_threads(n_trees, par_th);
    int n_sample_threads = calculate_num_threads(n_samples, par_th);
    // parallellize over trees
    if (n_tree_threads > 1 && parallel_predict && n_tree_threads > n_sample_threads){
        ...
        #pragma omp parallel
        {
            ...
            int thread_stop_tree_idx = (thread_id == n_tree_threads - 1) ? stop_tree_idx : thread_start_tree_idx + trees_per_thread;
            for (int sample_idx = 0; sample_idx < n_samples; ++sample_idx){
                getRepresentationFunc(dataset->obs, dataset->categorical_obs, sample_idx, edata, metadata, thread_start_tree_idx, thread_stop_tree_idx, n_leaves, opts, matrix);
            }        
        }
...
void Predictor::get_representation_matrix_over_trees(const float *obs, const char *categorical_obs, const int sample_idx, const ensembleData *edata, const ensembleMetaData *metadata, const int start_tree_idx, const int stop_tree_idx, const int n_leaves, std::vector<Optimizer*> opts, matrixRepresentation *matrix){
