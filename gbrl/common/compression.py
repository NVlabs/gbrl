...
def get_least_squares_W(C: th.Tensor, A: th.Tensor, V: th.Tensor, lambda_reg: float):
    CCT = C @ C.T
    ATA = A.T @ A
    CCTATA = CCT @ ATA
    del ATA
    ...
    eye = th.eye(CCT.size()[0], device=A.device)
    res = inv_mat @ CCTATA
    del inv_mat
    del CCTATA
    res = res @ (eye - CCT)
...
class TreeCompression:
    def __init__(self, k: int, gradient_steps: int, n_trees: int, n_leaves_per_tree: Union[np.ndarray, th.Tensor],
        ...
        self.compression = FirstK(k, n_trees, n_leaves_per_tree, n_leaves, output_dim, least_squares_W, 0, use_W,
                                  device, actor_critic) if method == 'first_k' else BestK(k, n_trees, n_leaves_per_tree,
                                                                                          n_leaves, output_dim,
                                                                                          least_squares_W, temperature,
                                                                                          lambda_reg, use_W, device,
                                                                                          actor_critic)
        self.optimizer = None
        self.device = device
        self.method = method
