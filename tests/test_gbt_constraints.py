...
# https://nvlabs.github.io/GBTModel/license.html
#
##############################################################################
import os
import shutil
...
class TestGBTConstraints(unittest.TestCase):
    ...
    def test_hierarchy_constraints_cpu(self):
        ...
        params = dict({"control_variates": False, "split_score_func": "Cosine",
        ...
        edata = model.learner._cpp_model.get_ensemble_data()
        ...
        _ = rmse_model(model, X, y, 1)
        edata = model.learner._cpp_model.get_ensemble_data()
        assert (edata['feature_indices'][:, 0].flatten() == 1).all(), \
            "Model should use feature index 1 for the root node"
        assert (edata['feature_indices'][:, 1].flatten() + (edata['mapping_numerics'] == True).sum() == 2).all(), \
            "Model should use feature index 2 for the second layer"
        assert (edata['feature_indices'][:, 2].flatten() == 0).all(), \
            "Model should use feature index 0 for the final layer"
    ...
    def test_threshold_constraints_cpu(self):
        ...
        _ = rmse_model(model, X, y, 1)
        edata = model.learner._cpp_model.get_ensemble_data()
        assert (edata['feature_indices'][:, 0].flatten() == 0).all() and (edata['feature_values'][:, 0] < -0.5).all(), \
            "Model should use feature index 1 for the root node"
        assert (edata['feature_indices'][:, 1].flatten() + (edata['mapping_numerics'] == True).sum() == 2).all() and \
            (edata['categorical_values'][:, 1] == b'B').all(), "Model should use feature index 2 for the second layer"
    ...
    def test_threshold_constraints_gpu(self):
        ...
        assert (edata['feature_indices'][:, 0].flatten() == 0).all() and (edata['feature_values'][:, 0] < -0.5).all(), \
            "Model should use feature index 1 for the root node"
        assert (edata['feature_indices'][:, 1].flatten() + (edata['mapping_numerics'] == True).sum() == 2).all() and \
               (edata['categorical_values'][:, 1] == b'B').all(), \
               "Model should use feature index 2 for the second layer"
